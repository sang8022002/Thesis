...
file_name1 = "Processing_Signal_PPG_PCG/240511/1715415199_114_65_90_khanh_0/PPG.TXT"
file_name2 = "Processing_Signal_PPG_PCG/240511/1715415199_114_65_90_khanh_0/PCG.TXT"
...
print("DBP:", DBP) 
# plot spo2
# dùng cửa sổ trượt tính trung bình spo2 cửa sổ là 3
# spo2 = np.array(spo2)
# spo2 = np.convolve(spo2, np.ones(3), 'valid') / 3
# # làm trong phần nguyên spo2
# spo2 = np.round(spo2)

# plt.plot(spo2)
# plt.xlabel('Number of spo2 values')
# plt.ylabel('SpO2(%)')
# plt.title('SpO2')
# plt.show()
...
# SBP = np.array(SBP)
# DBP = np.array(DBP)
# SBP = np.convolve(SBP, np.ones(3), 'valid') / 3
# DBP = np.convolve(DBP, np.ones(3), 'valid') / 3
# SBP = np.round(SBP)
# DBP = np.round(DBP)
# # vẽ 2 hình SBP và DBP
# fig, axs = plt.subplots(2, 1, sharex = True)
# axs[0].plot(SBP)
# axs[0].set_xlabel('Number of SBP values')
# axs[0].set_ylabel('SBP(mmHg)')
# axs[0].set_title('SBP')
# axs[1].plot(DBP)
# axs[1].set_xlabel('Number of DBP values')
